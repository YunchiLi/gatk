 ...
 * <code>--contig-ploidy-calls &lt;path&gt;</code></code>. The ploidy state of a contig is used as the <em>baseline</em>
 ...
 * probability (set by <code>--p-alt &lt;value&gt;</code>).</p>
 *
 ...
 * installed. If the user intends to run {@link GermlineCNVCaller} using one the official GATK Docker images, the python
 * environment is already set. Otherwise, the environment must be created and activated as described in the main GATK
 * README.md file.</p>
 *
 ...
 *      used later for CNV calling in one or more similarly-sequenced samples as mentioned earlier. Optionally, a
 *      previously obtained coverage model parametrization can be provided via <code>--model &lt;previous_model_path&gt;</code>
 ...
 *     (set by <code>--p-alt &lt;value&gt;</code>), prevalence of active (i.e. CNV-rich) intervals (set by <code>--p-active &lt;value&gt;</code>),
 *     the coherence length of CNV events and active/silent domains
 *     across intervals (set by <code>--cnv-coherence-length &lt;value&gt;</code> and <code>--class-coherence-length &lt;value&gt;</code>,
 *     respectively), and the typical scale of interval- and sample-specific unexplained variance (set by
 *     <code>--interval-psi-scale &lt;value&gt;</code> and <code>--sample-psi-scale &lt;value&gt;</code>, respectively).
 *     It is crucial to note that these hyperparameters are <em>not</em> universal and must be tuned for each
 *     sequencing protocol and properly set at runtime.</p></dd>
 *
